...
static int wusb3801x_otg_mode = false;
...
int wusb3801_read_thread(void *data)
{
	...
	while(1){
		...
		if (kthread_should_stop()){
			pr_info("wusb3801_read_thread: break from main thread \n");
			break;
		}
...
/************************************************************************
 * Hisense start, add otg and cc node for factory test.
 ************************************************************************/
int typec_get_otg_mode(void)
{
    int otg_mode = 0;

    otg_mode = wusb3801x_otg_mode;
    pr_info("@@@@otg_mode=%d\n", otg_mode);

    return otg_mode;
}

static int type_c_otg_show(struct seq_file *s, void *unused)
{
    int mode = 0;

    mode = typec_get_otg_mode();
...
static int type_c_otg_open(struct inode *inode, struct file *file)
{
	return single_open(file, type_c_otg_show, inode->i_private);
}

const struct file_operations type_c_otg_fops = {
	.open = type_c_otg_open,
	.read = seq_read,
	.llseek = seq_lseek,
	.release = single_release,
};

#define WUSB3801_STATUS_PLUG_ORIENTATION_MASK       0x03
#define WUSB3801_STATUS_STANDBY                     0x00
#define WUSB3801_STATUS_CC1_CONNECTED               0x01
#define WUSB3801_STATUS_CC2_CONNECTED               0x02
#define WUSB3801_STATUS_CC1_AND_CC2_CONNECTED       0x03

static int type_c_cc_show(struct seq_file *s, void *unused)
{
	struct wusb3801_chip *chip = s->private;
	int ret = 0, rc = 3;
	...
	pr_info("@@@@ret = 0x%x\n", ret);
    switch (ret & WUSB3801_STATUS_PLUG_ORIENTATION_MASK) {
    case WUSB3801_STATUS_CC1_CONNECTED:
        rc = 0;
        break;
    case WUSB3801_STATUS_CC2_CONNECTED:
        rc = 1;
        break;
    case WUSB3801_STATUS_CC1_AND_CC2_CONNECTED:
        rc = 2;
        break;
    default:
        rc = 3;
...
static int type_c_cc_open(struct inode *inode, struct file *file)
{
	return single_open(file, type_c_cc_show, inode->i_private);
}


const struct file_operations type_c_cc_fops = {
	.open = type_c_cc_open,
	.read = seq_read,
	.llseek = seq_lseek,
	.release = single_release,
...
static int type_c_attached_state_show(struct seq_file *s, void *unused)
{
	struct wusb3801_chip *chip = s->private;
	seq_printf(s, "%d\n", chip->attached);
...
static int type_c_attached_state_open(struct inode *inode, struct file *file)
{
	return single_open(file, type_c_attached_state_show, inode->i_private);
}

const struct file_operations type_c_attached_state_fops = {
	.open = type_c_attached_state_open,
	.read = seq_read,
	.llseek = seq_lseek,
	.release = single_release,
};

static void type_c_debugfs_init(struct wusb3801_chip *chip)
{
	int ret;
	
	ret = his_register_debugfs_file("typec_cc_orientation", S_IRUGO,
		chip, &type_c_cc_fops);
	if (ret < 0)
		pr_err("Couldn't create force_batt_psy_update file ret=%d\n", ret);
...
static ssize_t wusb3801_irq_flag_show(struct device *dev,
				struct device_attribute *attr,
				char *buf)
{
	struct i2c_client *client = to_i2c_client(dev);
	...
	mutex_lock(&chip->mlock);
	ret = snprintf(buf, PAGE_SIZE, "%u\n", chip->wusb3801_irq_flag);
	mutex_unlock(&chip->mlock);
	return ret;
}

static ssize_t wusb3801_irq_flag_store(struct device *dev,
				struct device_attribute *attr,
				const char *buff, size_t size)
{
	struct i2c_client *client = to_i2c_client(dev);
	...
	int rc = 0;
	rc = kstrtoul(buff, 10, &state);
	if (rc)
		goto out;
	chip->wusb3801_irq_flag = state;
	if(chip->wusb3801_irq_flag)
		enable_irq(chip->irq_gpio);
	rc = size;
out:
	pr_err("###chip->wusb3801_irq_flag:%d\n",chip->wusb3801_irq_flag);
		return rc;

}
DEVICE_ATTR(wusb3801_irq_flag, S_IRUGO | S_IWUSR, wusb3801_irq_flag_show, wusb3801_irq_flag_store);
...
static int wusb3801_create_devices(struct device *cdev)
{
	...
	ret = device_create_file(cdev, &dev_attr_wusb3801_irq_flag);
	if (ret < 0) {
		dev_err(cdev, "failed to create dev_attr_wusb3801_irq_flag\n");
		ret = -ENODEV;
		goto err8;
	}
#ifdef __TEST_CC_PATCH__
...
#endif /* __TEST_CC_PATCH__ */
err8:
	device_remove_file(cdev, &dev_attr_wusb3801_irq_flag);
...
static void wusb3801_attach(struct wusb3801_chip *chip)
{
	...
	dev_err(cdev, "sts[0x%02x], type[0x%02x]\n", status, type);
...
static void wusb3801_work_handler(struct work_struct *work)
{
	...
	dev_err(cdev,"%s WUSB3801_REG_STATUS : 0x%02x int_sts[0x%02x]\n",
		__func__, rc,int_sts);
...
static irqreturn_t wusb3801_interrupt(int irq, void *data)
{
	...
	__pm_wakeup_event(&chip->wake_src,msecs_to_jiffies(WUSB3801_WAKE_LOCK_TIMEOUT));
			...
			const struct i2c_device_id *id)
{
	...
	wakeup_source_init(&(chip->wake_src), "wusb3801_wake");
	...
	irq_set_status_flags(chip->irq_gpio, IRQ_NOAUTOEN);
